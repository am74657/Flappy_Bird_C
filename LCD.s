...
SSI0_DR_R               EQU   0x40008008 ;<- NOTE: The 0 is a zero, not the letter O.
...
busy
	...
	BNE busy				;2) If bit 4 is high, loop back to step 1 (wait for BUSY bit to be low)

	

	LDR R0,=DC				;3) Clear D/C=PA6 to zero
	LDR R1,[R1]
	BIC R1,#DC_DATA
	STR R1,[R0]
	...
	LDR R3,=SSI0_DR_R		;4) Write the command to SSI0_DR_R
	STR R0,[R3]
...
busy2
	...
	LDR R1,=SSI0_SR_R		;5) Read SSI0_SR_R and check bit 4, 
	LDR R0,[R1]
	AND R0,#SSI_SR_BSY
	CMP R0,#0
	

	BNE busy2				;6) If bit 4 is high, loop back to step 5 (wait for BUSY bit to be low)
...
full
	LDR R0,=SSI0_SR_R	;1) Read SSI0_SR_R and check bit 1, 
	LDR R1,[R0]
	AND R1,#SSI_SR_TNF
	...
	LDR R0,=DC			;3) Set D/C=PA6 to one
	LDR R1,[R0]
	ORR R1,#DC_DATA
	STR R1,[R0]
